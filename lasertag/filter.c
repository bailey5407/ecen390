...
#define BANDPASS_A_COEFFICIENT_AMOUNT 10
...
#define XQUEUE_NAME "xQueue"
#define YQUEUE_NAME "Jeremy"
...
//static array to keep track of oldest queue values
static queue_data_t oldestValue[BANDPASS_FILTERS_COUNT];
...
const static double IIR_A_Coefficients[BANDPASS_FILTERS_COUNT][BANDPASS_A_COEFFICIENT_AMOUNT] = {
   {-5.9637727070164033e+00,   1.9125339333078259e+01,  -4.0341474540744201e+01,   6.1537466875368885e+01,
   ...
   {-4.6377947119071452e+00,   1.3502215749461570e+01,  -2.6155952405269748e+01,   3.8589668330738313e+01,
   ...
   {-3.0591317915750946e+00,   8.6417489609637563e+00,  -1.4278790253808850e+01,   2.1302268283304322e+01,
   ...
   {-1.4071749185996751e+00,   5.6904141470697454e+00,  -5.7374718273676208e+00,   1.1958028362868866e+01,
   ...
   {8.2010906117760340e-01,   5.1673756579268613e+00,   3.2580350909220939e+00,   1.0392903763919197e+01,
   ...
   {2.7080869856154464e+00,   7.8319071217995475e+00,   1.2201607990980694e+01,   1.8651500443681531e+01,
   ...
   {4.9479835250075874e+00,   1.4691607003177591e+01,   2.9082414772101028e+01,   4.3179839108869274e+01,
   ...
   {6.1701893352279864e+00,   2.0127225876810343e+01,   4.2974193398071705e+01,   6.5958045321253508e+01,
   ...
   {7.4092912870072354e+00,   2.6857944460290113e+01,   6.1578787811202183e+01,   9.8258255839887198e+01,
   ...
   {8.5743055776347692e+00,   3.4306584753117896e+01,   8.4035290411037096e+01,   1.3928510844056828e+02,
...
void outputQueues_init(){
    ...
    for(uint8_t i = 0; i < BANDPASS_FILTERS_COUNT; i++){
        ...
        for(uint16_t j = 0; j < OUTPUTQUEUE_SIZE; j++){
...
double filter_iirFilter(uint16_t filterNumber){
    queue_data_t term_1 = QUEUE_INIT_VALUE;
    queue_data_t term_2 = QUEUE_INIT_VALUE;
    //calculate term 1
    for(uint16_t i = 0; i < YQUEUE_SIZE; i++){
        term_1 += IIR_B_Coefficients[filterNumber][i] * queue_readElementAt(&yQueue, (YQUEUE_SIZE - 1) - i);
    }
    //calculate term 2
    for(uint16_t i = 0; i < ZQUEUE_SIZE; i++){
        term_2 += IIR_A_Coefficients[filterNumber][i] * queue_readElementAt(&zQueues[filterNumber], (ZQUEUE_SIZE - 1) - i);
    }
    //output is difference of term 1 and term 2
    queue_data_t output = term_1 - term_2;
...
// Note that this function will probably need an
...
double filter_computePower(uint16_t filterNumber, bool forceComputeFromScratch, bool debugPrint) {
    ...
    // get outputQueue of given filternumber
    ...
    if (forceComputeFromScratch == true) {
        for (uint16_t i=0; i < OUTPUTQUEUE_SIZE; i++) {
        ...
        //3. Get the newest value from the output queuec
        queue_data_t newest_value = queue_readElementAt(outputQueue,OUTPUTQUEUE_SIZE - 1);

        //4. compute new_power
        power = prev_power - (oldestValue[filterNumber] * oldestValue[filterNumber]) + (newest_value * newest_value);
    ...
    oldestValue[filterNumber] = queue_readElementAt(outputQueue, 0);
...
void filter_getCurrentPowerValues(double powerValues[]) {
    for(uint16_t i = 0; i < POWER_SIZE; i++){
